...
GET /admin/system-settings                # 시스템 설정 조회
POST /admin/system-settings               # 시스템 설정 업데이트
POST /admin/system-backup                 # 시스템 백업 생성
GET /admin/system-logs                    # 시스템 로그 조회
DELETE /admin/clear-logs                  # 시스템 로그 삭제
```
...
### 시스템 설정 (v2.2.0 신규)
- **플랫폼 설정**: 플랫폼 이름, 설명, 최대 동시 세션, 세션 타임아웃 설정
- **언어 & 지역**: 기본 언어(한국어/영어/일본어), 시간대 설정
- **기능 토글**: 분석, 로깅, 캐릭터 필터링, 프로그램 차별화, 데모 모드 활성화/비활성화
- **유지보수 모드**: 시스템 일시 중단 및 사용자 메시지 설정
- **백업 관리**: 자동 백업 설정, 수동 백업 생성 (67,996개 레코드)
- **로그 관리**: 실시간 시스템 로그 조회, 로그 레벨별 색상 구분, 로그 삭제 기능

### API 관리
- **API 설정**: Base URL, OpenAI/Anthropic API 키 실시간 설정
- **연결 테스트**: API 서버 연결 상태 즉시 확인
- **사용량 모니터링**: AI 서비스, 데이터베이스 상태 실시간 추적
- **엔드포인트 탐색**: 전체 26개 API 엔드포인트 카테고리별 표시
...
### 시스템 설정 확장
1. 백엔드 `SystemSettings` Pydantic 모델에 새 필드 추가
2. MongoDB `system_settings` 컬렉션 스키마 업데이트
3. 프론트엔드 `settingsConfig` 상태에 새 필드 추가
4. UI에 새 설정 입력 컴포넌트 구현
...
### v2.2.0 (2025-09-03) - 시스템 설정 완전 구현
- ✅ **시스템 설정 관리**: 플랫폼 전반 설정 실시간 관리
- ✅ **유지보수 모드**: 시스템 일시 중단 및 메시지 설정
- ✅ **백업 시스템**: 자동/수동 백업 생성 (67,996개 레코드)
- ✅ **로그 관리**: 실시간 로그 조회, 레벨별 색상 구분, 삭제 기능
- ✅ **플랫폼 설정**: 언어, 시간대, 성능 파라미터 설정
- ✅ **기능 토글**: 분석, 로깅, 필터링 등 기능별 활성화/비활성화
...
- 프론트엔드: `http://localhost:3000/` 
- 백엔드: `http://127.0.0.1:8008/`
- 관리자 페이지: 상단 네비게이션 → "관리자" 
  - **시스템 설정**: "시스템 설정" 탭 (플랫폼 전반 관리)
  - **API 관리**: "API 관리" 탭 (API 설정 및 모니터링)
  - **통계 대시보드**: "개요" 탭 (실시간 시스템 상태)